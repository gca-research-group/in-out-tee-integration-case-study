...
- **Free time:** The metrics indicate a stark contrast in memory release times between the two configurations. Table 3 shows that freeing memory outside a compartment is highly efficient, with times ranging from 6 to 18 ms across block sizes. In contrast, Table 2 highlights the significant overhead when freeing memory inside a compartment, with times ranging from 89 ms for 100 MB blocks to 818 ms for 800 MB blocks. The high variability, as indicated by large standard deviations, is a result of the additional security and management overhead inherent to compartmentalised environments.
